...
function ghHeaders() {
  ...
  if (!token) {
    const err = new Error("PERSONAL_GITHUB_TOKEN is not configured on Vercel");
    err.status = 503;
    throw err;
  }
  return {
...
async function readSubscribers() {
  ...
  try {
    const data = await githubJson(url, { headers: ghHeaders() });
...
async function writeSubscribers(subscribers, sha, message) {
  const payload = {
    message,
  ...
  return githubJson(url, { method: "PUT", headers: ghHeaders(), body: JSON.stringify(payload) });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!saved) {
      return res.status(409).json({ error: "Could not save just then. Try once more." });
    }
    ...
    return res.status(status).json({
      error:
        status === 503
          ? "Newsletter is not live yet. Add PERSONAL_GITHUB_TOKEN on Vercel."
