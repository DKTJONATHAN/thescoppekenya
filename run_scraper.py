...
def get_target_urls():
    ...
    try:
        with sync_playwright() as p:
            ...
            for a in soup.find_all('a', href=True):
                ...
                if (len(slug) > 20 and '-' in slug and not any(x in path for x in ['/author/','/tag/','/page/','/feed/','/comment-','/category/'])):
                    urls.add(full_url)
            browser.close()
    except: pass
    ...
    return sorted(list(urls), key=date_key, reverse=True)[:12]
...
def gemini_generate(prompt, use_search=False):
...
for link in article_links:
    ...
    if text and len(text) > 400:
        full_raw_text, target_image, target_title, final_hash = text, img, title or "Entertainment News", u_hash; break
...
title_prompt = (
    ...
    "ARTICLE SNIPPET:\n" + full_raw_text[:1500] + "\n"
    "Write a single news headline for this article.\n"
    "Rules: Plain reporting tone like a newspaper front page. No dates or years in the title. No numbers at the start. No clickbait. No brackets. No hype words. Just a clean factual sentence that tells the reader exactly what happened.\n"
    "Output ONLY JSON: {\"final_title\": \"Your Title Here\"}"
)

title_text = gemini_generate(title_prompt)
...
lines = [
    ...
    'tags: ["kenya entertainment", "celebrity news", "trending"]',
    'canonical: "https://zandani.co.ke/article/' + slug + '"',
    'date: "' + publish_timestamp + '"',
    'dateModified: "' + publish_timestamp + '"',
    'focusKeyword: "' + final_title.replace('"', "'")[:65] + '"',
    'schema: "NewsArticle"',
    "---",
    article_text,
    ""
...
with open(filepath, "w", encoding="utf-8") as f: f.write(final_file)
