...
const AUTHOR_BIOS: Record<string, string> = {
  ...
  "mutheu ann": "Plugged into Kenya's entertainment circuit. If a celebrity breathes wrong, Mutheu Ann notices — and she will write about it.",
  ...
  "wanjiku kuria": "Nairobi gossip desk. Wanjiku tracks the tea from ghafla streets to the DMs — receipts first, noise second.",
  "martin kihara": "Showbiz beat for Kenyan stars. Martin covers the red carpets, the fallouts, and the comebacks without the fluff.",
};
...
function stripArticleBoilerplateHtml(html: string): string {
  ...
  if (typeof DOMParser === "undefined") {
    ...
    sectionPatterns.forEach((pattern) => { cleaned = cleaned.replace(pattern, ""); });
  ...
  doc.body.querySelectorAll("h1, h2, h3, h4, h5, h6, p, li, blockquote").forEach((node) => {
    ...
    if (!normalized) { node.remove(); return; }
    if (isLegacyArticleBlock(normalized)) { node.remove(); return; }
    if (seen.has(normalized)) { node.remove(); return; }
    seen.add(normalized);
  });
...
export default function ArticlePage() {
  ...
  const wovenHtml = useMemo(() => {
    ...
    sameCategoryLinks.forEach(link => {
      const words = link.title.replace(/[^\w\s'-]/g, ' ').split(/\s+/).filter(Boolean);
  ...
  const canonicalUrl = useMemo(() => post ? `${SITE_URL}/article/${post.slug}` : SITE_URL, [post?.slug]);
  const metaDescription = useMemo(() => {
    if (!post) return "";
    const fk = (post.focusKeyword || "").trim();
    let base = post.excerpt || "";
    if (fk && !base.toLowerCase().includes(fk.toLowerCase())) base = `${fk}: ${base}`;
  ...
  const articleSchema = useMemo(() => post ? {
    ...
    "image": [{ "@type": "ImageObject", "url": postOgImage, "width": 1200, "height": 630 }],
    "datePublished": isoPublished,
    "dateModified": post.dateModified ? new Date(post.dateModified).toISOString() : isoPublished,
    "author": { "@type": "Person", "name": post.author, "url": `${SITE_URL}/author/${post.author.toLowerCase().replace(/\s+/g, '-')}` },
    "publisher": {
    ...
    "isPartOf": { "@type": "WebSite", "name": "Zandani", "url": SITE_URL },
    ...
    "copyrightHolder": { "@type": "Organization", "name": "Za Ndani" },
  ...
  return (
    <Layout>
      ...
      <section className="relative w-full bg-zinc-950 overflow-hidden" style={{ minHeight: 540 }}>
        ...
        <div className="relative z-10 pt-6 px-4">
          <div className="container max-w-5xl mx-auto">
            <div className="flex items-center gap-2 text-xs text-zinc-500">
              ...
              <span>></span>
              <Link to={`/category/${post.category.toLowerCase()}`} className="hover:text-zinc-300 transition-colors capitalize">{post.category}</Link>
              <span>></span>
        ...
        <div className="relative z-10 container max-w-5xl mx-auto px-4 pb-14 pt-8">
          <div className="max-w-3xl space-y-5">
            ...
            <h1 className="article-headline text-3xl md:text-5xl lg:text-6xl font-serif font-black text-white leading-[1.08] tracking-tight">{post.title}</h1>
            <p className="article-excerpt text-zinc-300 text-base md:text-lg leading-relaxed max-w-2xl font-light">{post.excerpt}</p>
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm pt-1">
              <Link to={`/author/${post.author.toLowerCase().replace(/\s+/g, '-')}`} className="flex items-center gap-2 group" rel="author">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-black flex-shrink-0 ${authorColor}`}>{authorInitials}</div>
                <span className="text-white font-bold text-sm group-hover:text-primary transition-colors">{post.author}</span>
              </Link>
              <span className="text-zinc-500">·</span>
              <time dateTime={isoPublished} className="flex items-center gap-1.5 text-zinc-400"><Calendar className="w-3.5 h-3.5" /> {formattedDate}</time>
              ...
              <span className="text-zinc-500">·</span>
              <span className="flex items-center gap-1.5 text-primary font-semibold"><Eye className="w-3.5 h-3.5" />{currentPostViews > 999 ? `${(currentPostViews / 1000).toFixed(1)}k` : currentPostViews} views</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container max-w-6xl mx-auto px-4 py-10">
          <div className="grid lg:grid-cols-12 gap-10">
            ...
            <article className="lg:col-span-8 min-w-0">
              ...
              <div className="mt-10"><LiveUpdatesTimeline maxItems={10} title="Live Updates" /></div>
              <div className="mt-10 flex justify-center border border-divider bg-muted/10 p-3"><AdUnit type="horizontal" /></div>
      ...
      {post && <StickyMobileShare title={post.title} url={shareUrl} />}
    </Layout>
  );
}
